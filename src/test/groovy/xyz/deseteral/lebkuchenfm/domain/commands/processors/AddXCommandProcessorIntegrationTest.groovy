...
class AddXCommandProcessorIntegrationTest extends IntegrationSpecification {
    def "should add sound with spaces in it's name"() {
        given:
        def firstAddRequest = slackCommandRequest('/fm', 'addx super test sound|super-test-sound.com')
    ...
    def 'should not add sounds with the same name'() {
        given:
        def firstAddRequest = slackCommandRequest('/fm', 'addx test|first-url.com')
        ...
        and:
        def secondAddRequest = slackCommandRequest('/fm', 'addx test|second-url.com')
